...
<script>
    $( document ).ready(function() {
        $('li.dataporten').html(
          "<center>" +
            "<a rel='nofollow' target='_top' href='/auth/dataporten'>" +
...
Remember to enable the `Enable custom header` option below the code view. 

With this in place, a more palatable login button for Dataporten is made available.

**Callback URL**

NodeBB may pick up your forum's domain/IP as running on localhost, something you may want to change if your client redirect should be to a public server.

If the forum is served with Nginx, some instructions on how to change from localhost to actual domain [may be found here](https://community.nodebb.org/topic/4025/how-to-change-localhost-4567-to-actual-domain-name/8).

I found that a small edit in `config.js` did the trick, by adding a `bind_address` field and editing the existing `port` value:

```json
    "port": "http://IP_OR_DOMAIN:4567",
    "bind_address": "http://localhost:4567",
```
...
A `Social Authentication` menu, in which your Client ID and Secret may be added, should now be available. The `callback` URL is also displayed here (which will need to be entered in your registered Dataporten client).
